...
Durable caveats for cloud-agent VMs; the startup update script already runs `corepack pnpm install`. Standard commands live in `CONTRIBUTING.md` / root `package.json` scripts; only cloud-specific gotchas below.

- Node: `/exec-daemon/node` is pinned to v22.14.0 (below the `>=22.19.0` engine floor) and is prepended to `PATH`, so it wins over nvm. Node 24 is installed via nvm and symlinked into `/usr/local/cargo/bin` (which sits ahead of `/exec-daemon` in `PATH`) as `node`/`npm`/`npx`/`corepack`. If `node --version` ever reports < 22.19, re-link: `for b in node npm npx corepack; do ln -sf "$(nvm which 24 2>/dev/null || echo $HOME/.nvm/versions/node/v24.18.0/bin)/$b" /usr/local/cargo/bin/$b; done` (or `ln -sf ~/.nvm/versions/node/v24*/bin/<b> /usr/local/cargo/bin/<b>`). A matching `PATH` guard also lives in `~/.bashrc`.
- No LLM provider keys are present in this env. To run real agent turns without keys, use the QA mock lane (`extensions/qa-lab`): one-off `pnpm openclaw qa manual --provider-mode mock-openai --message "..."`; standalone server `pnpm openclaw qa mock-openai --port <p>` exposes an OpenAI-Responses-compatible mock at `http://127.0.0.1:<p>/v1`. The mock always replies with a fixed "Protocol note: acknowledged..." string; that is expected, not a bug.
- Dev gateway: `pnpm gateway:dev` (sets `OPENCLAW_SKIP_CHANNELS=1`) or `pnpm gateway:watch`. It uses the `dev` profile at `~/.openclaw-dev/openclaw.json` (workspace `~/.openclaw/workspace-dev`, agent id `dev`), auto-created on first run. The HTTP/Control-UI port is assigned dynamically (not always 18789); confirm the live port with `curl -s http://127.0.0.1:<p>/healthz` (look for `{"ok":true}`) after startup.
- Control UI is served by the gateway from `dist/control-ui`; run `pnpm ui:build` once so it exists. The UI shows a login gate — enter the gateway token (`gateway.auth.token`, or a runtime token from the startup log). Set a persistent one with `openclaw --profile dev config set gateway.auth.mode token` + `... gateway.auth.token <token>`, then restart the gateway.
- To make in-browser WebChat reply without keys: run the mock server, then in `~/.openclaw-dev/openclaw.json` set `models.providers.openai` to `{ baseUrl: "http://127.0.0.1:<p>/v1", apiKey: "test", api: "openai-responses", request: { allowPrivateNetwork: true }, models: [{ id: "gpt-5.5", ... }] }`, add an `auth.profiles` `api_key` entry for `openai`, and stage the key in `~/.openclaw-dev/agents/dev/agent/auth-profiles.json`. Restart the gateway. (`agent`/singular is not a valid top-level config key; the default agent model is already `openai/gpt-5.5`.)
- Verification here defaults to remote (Crabbox/Testbox) per this file. Local fallbacks that work in-VM: tests `node scripts/run-vitest.mjs <path>`, lint `node scripts/run-oxlint.mjs <path>` (first run is slow, ~60s), format `pnpm exec oxfmt --check --threads=1 <path>`. Running the CLI (`pnpm openclaw ...`) auto-compiles a stale `dist/` on first invocation.
